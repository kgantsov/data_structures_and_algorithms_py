...
class TestBloomFilter(TestCase):
    ...
    def test_size(self):
        b = BloomFilter(probability=0.1, n=2)
        self.assertEqual(len(b.hash_table), 10)

        b = BloomFilter(probability=0.1, n=5)
        self.assertEqual(len(b.hash_table), 24)

        b = BloomFilter(probability=0.1, n=10)
        self.assertEqual(len(b.hash_table), 48)

        b = BloomFilter(probability=0.1, n=100)
        self.assertEqual(len(b.hash_table), 480)

        b = BloomFilter(probability=0.1, n=1000)
        self.assertEqual(len(b.hash_table), 4793)

        b = BloomFilter(probability=0.01, n=2)
        self.assertEqual(len(b.hash_table), 20)

        b = BloomFilter(probability=0.01, n=5)
        self.assertEqual(len(b.hash_table), 48)

        b = BloomFilter(probability=0.01, n=10)
        self.assertEqual(len(b.hash_table), 96)

        b = BloomFilter(probability=0.01, n=100)
        self.assertEqual(len(b.hash_table), 959)

        b = BloomFilter(probability=0.01, n=1000)
        self.assertEqual(len(b.hash_table), 9586)

        b = BloomFilter(probability=0.0005, n=2)
        self.assertEqual(len(b.hash_table), 32)

        b = BloomFilter(probability=0.0005, n=5)
        self.assertEqual(len(b.hash_table), 80)

        b = BloomFilter(probability=0.0005, n=10)
        self.assertEqual(len(b.hash_table), 159)

        b = BloomFilter(probability=0.0005, n=100)
        self.assertEqual(len(b.hash_table), 1583)

        b = BloomFilter(probability=0.0005, n=1000)
        self.assertEqual(len(b.hash_table), 15821)

        b = BloomFilter(probability=0.0005, n=5000)
